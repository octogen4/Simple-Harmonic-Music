...
def play_with_arpeggio(melody_freq, arpeggio_name, duration):
    """播放立体声：左声道主旋律，右声道琶音"""
    # 生成左声道（主旋律）和右声道（琶音）
    left_channel = make_melody_sound(melody_freq, duration)
    right_channel = make_arpeggio_sound(arpeggio_patterns[arpeggio_name], duration)
    ...
    # 创建立体声缓冲区：左声道 | 右声道
    stereo_buf = np.column_stack((left_channel, right_channel))
    
    # 转换为int16格式
    stereo_buf = (stereo_buf * 32767).astype(np.int16)
...
# 第五段：Twinkle twinkle little star（重复）
play_with_arpeggio(freqs['C5'], 'C', note4)
play_with_arpeggio(freqs['C5'], 'C', note4)
play_with_arpeggio(freqs['G5'], 'C', note4)
play_with_arpeggio(freqs['G5'], 'C', note4)
...
# 第六段：How I wonder what you are（重复）
play_with_arpeggio(freqs['F5'], 'F', note4)
play_with_arpeggio(freqs['F5'], 'F', note4)
play_with_arpeggio(freqs['E5'], 'C', note4)
play_with_arpeggio(freqs['E5'], 'C', note4)
