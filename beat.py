...
freq1 = 500.1  # frequency of the sine wave in Hz
freq2 = 500
buf1 = np.sin(2 * np.pi * np.arange(fs * duration) * freq1 / fs).astype(np.float32)
buf2 = np.sin(2 * np.pi * np.arange(fs * duration) * freq2 / fs).astype(np.float32)
